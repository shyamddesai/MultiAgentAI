...
class SophisticatedKeywordGeneratorTool(BaseTool):
    ...
    def _run(self, topic: str) -> list:
        ...
        entities = [ent.text for ent in doc.ents if ent.label_ in ["ORG", "GPE", "PRODUCT", "EVENT"] and (
                    'oil' in ent.text.lower() or 'gas' in ent.text.lower())]
        # Debug: Print extracted entities
        print(f"Extracted Entities: {entities}")

        # Extract relevant noun chunks
        noun_chunks = [chunk.text for chunk in doc.noun_chunks if chunk.text.lower() not in STOP_WORDS and (
                    'oil' in chunk.text.lower() or 'gas' in chunk.text.lower())]
